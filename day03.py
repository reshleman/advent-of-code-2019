...
def points_for_range(start, direction, distance):
    if direction == 'U':
        return [
            Point(start.x, y)
            for y
            in range(start.y + 1, start.y + distance + 1)
        ]
    if direction == 'D':
        return [
            Point(start.x, y)
            for y
            in reversed(range(start.y - distance, start.y))
        ]
    if direction == 'R':
        return [
            Point(x, start.y)
            for x
            in range(start.x + 1, start.x + distance + 1)
        ]
    if direction == 'L':
        return [
            Point(x, start.y)
            for x
            in reversed(range(start.x - distance, start.x))
        ]
    raise ValueError(f"Unknown direction: {direction}")


def wire_from_instructions(instructions):
    wire = [PORT]
    ...
    for instruction in instructions:
        ...
        points = points_for_range(wire[-1], direction, distance)
        wire.extend(points)
...
def part1():
    wire_1 = set(wire_from_instructions(INPUT[0].split(',')))
    wire_2 = set(wire_from_instructions(INPUT[1].split(',')))
    ...
    for point in intersections:
        ...
        distance = manhattan_distance(PORT, point)
...
def part2():
    wire_1 = wire_from_instructions(INPUT[0].split(','))
    wire_2 = wire_from_instructions(INPUT[1].split(','))
    ...
    for point in intersections:
        ...
        steps = wire_1.index(point) + wire_2.index(point)
